...
const PaymentModal = ({ content }) => {
    ...
    const onChange = (e) => {
        SetPayment({ methodName: e.target.value });
    }

    const UpdateSales = async () => {
        const ReceiptNo = "#" + Date.now();
        const PaymentMethod = Payment.methodName;
        const TotalAmount = TotalPrice;
        const itemId = await content?.map((ele) => ele.id);

        await axios({
            method: "POST",
            url: `${process.env.REACT_APP_API}/api/AddSales`,
            headers: {
                    ...
                    <div className="flex-col justify-center  bg-white py-12 px-14 border-2 border-grey-500 rounded-xl ">
                        ...
                        <div class="flex-1 max-w-5xl md:max-w-5xl mx-auto p-1 mt-4">
                            ...
                            <ul class="grid grid-cols-1">
                                <li className='w-full md:w-[38rem] md:m-2'>
                                    ...
                                    <ul class="items-center w-full text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                            <div class="flex items-center pl-3">
                                                <input id="horizontal-list-radio-license" defaultChecked="true" onChange={onChange} type="radio" value="Cash" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                        ...
                                        <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                            <div class="flex items-center pl-3">
                                                <input id="horizontal-list-radio-id" onChange={onChange} value="Card/Online payment" type="radio" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                    ...
                                    <button type="submit" onClick={Global} class="ml-2 mt-5 text-white bg-[#04d1b2] focus:ring-1 focus:outline-none  font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Generate bills</button>
                                    <hr />
